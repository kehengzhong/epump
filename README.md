...
* [三. ePump框架工作原理](#三-ePump框架工作原理)
    * [3.1 ePump基础数据结构](#31-ePump基础数据结构)
        * [3.1.1 设备（iodev_t）](#311-设备（iodev_t）)
        * [3.1.2 定时器（iotimer_t）](#312-定时器（iotimer_t）)
        * [3.1.3 事件（ioevent_t）](#313-事件（ioevent_t）)
    * [3.2 ePump多线程架构](#32-epump多线程架构)
* [四. ePump框架工作模型](#四-epump框架工作模型)
    * [4.1 快业务模型 -- 没有worker线程，只有ePump线程](#41-快业务模型----没有worker线程，只有ePump线程)
    * [4.2 复合业务模型 -- 少数ePump线程，大多数worker线程](#42-复合业务模型----少数ePump线程，大多数worker线程)
* [五. ePump框架中的文件描述符FD](#五-epump框架中的文件描述符fd)
* [六. ePump框架的回调（Call Back）机制](#六-epump框架的回调（call-back）机制)
* [七. ePump框架的调度（Scheduling）机制](#七-epump框架的调度（scheduling）机制)
...
* [八. ePump框架中惊群效应的处理机制](#八-epump框架中惊群效应的处理机制)
    * [8.1 惊群效应（Thundering Herd Problem）](#81-惊群效应（thundering-herd-problem）)
