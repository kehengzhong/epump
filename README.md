...
* 1. 事件调度的基础算法是低负载优先算法，即选择当前负载最低的worker工作线程，并将事件派发到该线程的事件队列中。

* 2. 对于同一个iodev_t设备产生的后续所有ioevent_t事件，都会以pipeline方式调度到同一个worker线程中。

* 3. 对于同一个iodev_t设备连续产生的同一类型的ioevent_t事件，如果还在同一个worker工作线程的FIFO事件队列中，尚未被取走执行，那么后续的这样同设备同类型事件就会被抛弃。

* 4. 由哪个worker线程启动的iotimer_t定时器，其超时事件最终仍然由该worker工作线程处理。

* 5. 如果ePump框架中没有启动worker工作线程，则选择当前负载最低的ePump线程，并将事件派发到该线程的事件队列。
